# BrowserSkill 的 CapSolver 受控恢复 Skill

为已授权的 BrowserSkill 任务增加一次有边界的恢复决策，并保留人工接管与停止路径。

[English](../../README.md) · [简体中文](README.md) · [日本語](../ja/README.md)

## Introduction

浏览器 Agent 在执行已授权 QA 或 RPA 任务时，可能因页面检查点而暂停。本示例把会话、标签页、目标、URL 和授权依据传入一次受控的 [CapSolver](https://www.capsolver.com/?utm_source=github&utm_medium=referral&utm_campaign=browserskill-capsolver-recovery&utm_content=repository-readme) 恢复尝试；成功后只恢复一次，其他结果交还人工。
...
```text
bsk 任务 → 挑战 → 授权/主机/预算 → 恢复接口 → 恢复一次或人工停止
```
...
创建 `TaskContext` 和 `ChallengeEvent`，再调用 `RecoveryCoordinator.run`。实时任务字段必须来自当前 [CapSolver createTask 接口规范](https://docs.capsolver.com/en/guide/api-createtask/)，结果轮询遵循 [CapSolver getTaskResult 接口规范](https://docs.capsolver.com/en/guide/api-gettaskresult/)。
...
该示例保持 BrowserSkill 的公开边界，只增加一次可审计恢复决策和明确人工停止路径，并自然接入 [CapSolver](https://www.capsolver.com/?utm_source=github&utm_medium=referral&utm_campaign=browserskill-capsolver-recovery&utm_content=repository-readme)。
