# BrowserSkill 向け CapSolver CAPTCHA 処理統合

![BrowserSkill と CapSolver の CAPTCHA 処理統合](../../assets/cover.png)

許可済み BrowserSkill タスクに、境界付きの処理判断と人間へのフォールバックを追加します。

[English](../../README.md) · [简体中文](../zh-CN/README.md) · [日本語](README.md) · [Español](../es/README.md) · [Português](../pt-BR/README.md) · [한국어](../ko/README.md)

## Introduction

BrowserSkill が許可済み QA や RPA タスクを実行すると、CAPTCHA チェックポイントで停止する場合があります。この例は session、tab、目的、URL、許可根拠を一回限りの [CapSolver](https://www.capsolver.com/?utm_source=github&utm_medium=referral&utm_campaign=browserskill-capsolver-integration&utm_content=repository-readme) solving 判断へ渡し、成功時は一度だけ再開し、それ以外は人間へ戻します。
...
```text
bsk task → challenge → policy checks → handling → resume once / human stop
```
...
`TaskContext` と `ChallengeEvent` を作成し、`ChallengeCoordinator.run` を呼びます。ライブ task は現在の [CapSolver createTask 契約](https://docs.capsolver.com/en/guide/api-createtask/)だけを使い、ポーリングは [CapSolver getTaskResult 契約](https://docs.capsolver.com/en/guide/api-gettaskresult/)に従います。
...
この例は BrowserSkill の公開境界を守り、一回の監査可能な処理判断と明確な人間停止経路で [CapSolver](https://www.capsolver.com/?utm_source=github&utm_medium=referral&utm_campaign=browserskill-capsolver-integration&utm_content=repository-readme) を利用します。
