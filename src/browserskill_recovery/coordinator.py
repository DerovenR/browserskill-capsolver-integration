...
class RecoveryCoordinator:
    def __init__(self, gateway: RecoveryGateway, clock: Callable[[], float] = time.monotonic) -> None:
    ...
    def run(self, context: TaskContext, event: ChallengeEvent) -> RecoveryDecision:
        ...
        if context.max_attempts < 1:
            return self._decision(context, "request_human_help", "recovery budget is exhausted", 0)
        ...
        try:
            ...
            return self._decision(context, "request_human_help", "recovery timed out", 1)
        except Exception:
            return self._decision(context, "request_human_help", "recovery provider error", 1)
        if result.status == "ready":
            return self._decision(context, "resume_once", "recovery result is ready", 1, result.solution)
        return self._decision(
            context, "request_human_help", f"recovery stopped: {result.error_code or result.status}", 1
        )
