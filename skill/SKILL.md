---
name: browserskill-capsolver-integration
description: Apply one bounded handling attempt to an authorized BrowserSkill task, then use BrowserSkill human help or stop.
---

# BrowserSkill CapSolver handling companion
...
4. Call `ChallengeCoordinator.run`. Do not invent provider task fields; pass only a task contract verified against current official CapSolver documentation.
...
6. On every other handling outcome, build the exact foreground fallback with `build_request_help_command`. Treat `cancelled`, `timed_out`, or `disabled` as stop conditions. After `continued` or `completed`, take one fresh snapshot.
